...
Data: 13/09/2026. Escopo aprovado: itens SEC-01 e SEC-02 do relatório.

## Estado da mudança

Implementação local, sem deploy e sem alterar variáveis, dados ou credenciais do Railway. Os demais achados do relatório continuam fora deste trabalho.

### Chaves independentes

| Variável | Uso | Efeito da troca |
|---|---|---|
| `PAINEL_SECRET` | Assinar links e sessões do `!painel`; fica no servidor | Invalida links e sessões assinados pela chave antiga |
| `PAIRING_SECRET` | Abrir a página de QR `/pareamento?token=...` | Invalida a URL de QR anterior, sem invalidar o painel |
...
O QR continua disponível apenas enquanto há pareamento pendente. Todas as respostas dessa rota, inclusive recusas, usam `no-store`, `noindex`, `no-referrer` e `nosniff`. Os logs da rota não incluem chaves ou URLs completas.

**Limite desta correção:** a nova URL de pareamento ainda contém uma credencial estática, exclusiva do QR. Pode ficar no histórico, em favoritos ou em logs de infraestrutura. Não compartilhar nem registrar a URL. A autorização temporária e de uso único sugerida no relatório não foi implementada neste escopo; a separação impede que essa URL revele a chave de assinatura do painel.
...
6. Se houver QR pendente, abrir a página usando somente a nova `PAIRING_SECRET`. Sem QR pendente, o 404 informando que o bot já está conectado é esperado. Não apagar `creds.json`, não desparear o WhatsApp e não revincular impressoras para testar esta troca.
...
**Rotações futuras:** trocar somente `PAIRING_SECRET` quando vazar a URL de QR; trocar `PAINEL_SECRET` quando houver suspeita sobre a chave de assinatura. A ausência de `PAIRING_SECRET` pode ser usada para manter a página de QR desligada quando não necessária.
...
**Limites:** esta validação não é antivírus, não remove metadados da foto original e não impede instruções maliciosas visíveis nos pixels (prompt injection). Os controles de prompt injection do achado SEC-06 permanecem um trabalho separado. Limites de bytes/pixels reduzem abuso de recursos, sem substituir monitoramento e atualização contínua das bibliotecas.

## Verificação local e teste após publicação

Testes locais, sem serviços externos nem uso de créditos de IA:

Resultado em 13/09/2026: **84 suítes passaram** (`npm test`), incluindo as novas verificações. `npm audit --omit=dev` retornou **0 vulnerabilidades conhecidas**. `npm ls sharp --all` confirmou uma única versão `0.35.4`, compartilhada pelo app e pelo Baileys; o runtime local informou libheif `1.23.2`. A comparação do lock confirmou que só sharp e seus binários de plataforma mudaram de versão.

- `node test/pareamentotest.js`: ausência/força/independência das chaves, recusa da credencial antiga, rotação de QR e painel, sessão forjada com a chave de QR recusada, cabeçalhos de segurança.
