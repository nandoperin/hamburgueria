...
--   awaiting_review  (legado) comprovante esperando liberacao do dono
--   paid             comprovante chegou, ou o dono liberou sem ele  <- imprime
--   cash_due         cash a cobrar na entrega/retirada               <- imprime
--   printed          a comanda saiu no papel
--   delivered        entregue
--   rejected         o dono recusou o pagamento (!recusar)
--   cancelled        cancelado
--
-- getNextPrintableOrder() busca 'paid' e 'cash_due'. No Zelle, o print do
-- comprovante poe o pedido em 'paid' e a comanda sai na hora; a conferencia do
-- dinheiro fica no pagamento (payments.status 'awaiting_review' ate o
-- !liberar gravar 'paid' e quem conferiu).

-- Pagamentos (Zelle)
--
-- Zelle nao tem webhook nem API de estorno: nada externo confirma o pagamento,
-- e nada desfaz. Por isso a conferencia e humana e fica registrada aqui — quem
-- conferiu e quando. Ela vem depois da comanda, que sai com o comprovante.
